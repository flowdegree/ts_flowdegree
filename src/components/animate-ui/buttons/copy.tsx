  ...
  'inline-flex items-center justify-center cursor-pointer rounded-md transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive',
  {
    variants: {
      variant: {
        ...
        ghost:
          'bg-transparent hover:bg-black/5 dark:hover:bg-white/10 hover:text-accent-foreground',
      },
      size: {
...
function CopyButton({
  ...
  return (
    <motion.button
      ...
      onClick={handleCopy}
      aria-label={localIsCopied ? "Code copied to clipboard" : "Copy code to clipboard"}
      title={localIsCopied ? "Code copied to clipboard" : "Copy code to clipboard"}
      {...props}
    >
      ...
      <span className="sr-only">
        {localIsCopied ? "Code copied to clipboard" : "Copy code to clipboard"}
      </span>
    </motion.button>
  );
}
